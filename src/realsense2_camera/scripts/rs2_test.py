...
def ImuGetData(rec_filename, topic):
    ...
    for topic, msg, t in bag.read_messages(topics=topic):
        ...
        if res["value"] is None:
            ...
            res["max_diff"] = [
                max(diff[x], res["max_diff"][x]) for x in range(len(diff))
            ]
...
def ImuTest(data, gt_data):
    ...
    try:
        ...
        msg = "original accel: Expect max diff of %.3f. Got %.3f." % (
            gt_data["max_diff"].max(),
            max_diff,
        )
        print(msg)
        ...
        diff = v_data - v_gt_data
        max_diff = abs(diff).max()
        msg = "rotated to ROS: Expect max diff of %.3f. Got %.3f." % (
            gt_data["ros_max_diff"].max(),
            max_diff,
        )
        print(msg)
...
def ImageColorTest(data, gt_data):
    ...
    try:
        ...
        msg = "Expect %d channels. Got %d channels." % (
            gt_data["num_channels"],
            channels[0],
        )
        print(msg)
        if len(channels) > 1 or channels[0] != gt_data["num_channels"]:
            return False, msg
        msg = "Expected all received images to be the same shape. Got %s" % str(
            set(data["shape"])
        )
        print(msg)
        if len(set(data["shape"])) > 1:
            return False, msg
        msg = "Expected shape to be %s. Got %s" % (
            gt_data["shape"],
            list(set(data["shape"]))[0],
        )
        print(msg)
        if (np.array(list(set(data["shape"]))[0]) != np.array(gt_data["shape"])).any():
            return False, msg
        msg = "Expected header [width, height, step] to be %s. Got %s" % (
            gt_data["reported_size"],
            list(set(data["reported_size"]))[0],
        )
        print(msg)
        if (
            np.array(list(set(data["reported_size"]))[0])
            != np.array(gt_data["reported_size"])
        ).any():
            return False, msg
        msg = "Expect average of %.3f (+-%.3f). Got average of %.3f." % (
            gt_data["avg"].mean(),
            gt_data["epsilon"],
            np.array(data["avg"]).mean(),
        )
        print(msg)
        if (
            abs(np.array(data["avg"]).mean() - gt_data["avg"].mean())
            > gt_data["epsilon"]
        ):
            return False, msg

        msg = "Expect no holes percent > %.3f. Got %.3f." % (
            gt_data["ok_percent"]["value"] - gt_data["ok_percent"]["epsilon"],
            np.array(data["ok_percent"]).mean(),
        )
        print(msg)
        if (
            np.array(data["ok_percent"]).mean()
...
def PointCloudTest(data, gt_data):
    ...
    msg = "Expect image size %d(+-%d), %d. Got %d, %d." % (
        gt_data["width"][0],
        gt_data["width"][1],
        gt_data["height"][0],
        width,
    ...
    print(msg)
    if (
        abs(width - gt_data["width"][0]) > gt_data["width"][1]
        or height != gt_data["height"][0]
    ):
        return False, msg
    mean_pos = np.array([xx[:3] for xx in data["avg"]]).mean(0)
    msg = "Expect average position of %s (+-%.3f). Got average of %s." % (
        gt_data["avg"][0][:3],
        gt_data["epsilon"][0],
        mean_pos,
    )
    ...
    msg = "Expect average color of %s (+-%.3f). Got average of %s." % (
        gt_data["avg"][0][3:],
        gt_data["epsilon"][1],
        mean_col,
    )
...
def staticTFTest(data, gt_data):
    for couple in gt_data.keys():
        if data[couple] is None:
            ...
            return False, msg
        if any(
            abs((np.array(data[couple][0]) - np.array(gt_data[couple][0]))) > 1e-5
        ) or any(
            abs((np.array(data[couple][1]) - np.array(gt_data[couple][1]))) > 1e-5
        ):
...
test_types = {
    "vis_avg": {
        "listener_theme": "colorStream",
        "data_func": ImageColorGetData,
        "test_func": ImageColorTest,
    },
    "depth_avg": {
        "listener_theme": "depthStream",
    ...
    "align_depth_ir1": {
        "listener_theme": "alignedDepthInfra1",
        "data_func": ImageDepthGetData,
        "test_func": ImageColorTest,
    },
    "align_depth_color": {
        "listener_theme": "alignedDepthColor",
    ...
    "static_tf": {
        "listener_theme": "static_tf",
        "data_func": lambda x: {
            ("camera_link", "camera_color_frame"): (
                [
                    -0.00010158783697988838,
                    0.014841210097074509,
                    -0.00022671300393994898,
                ],
                [
                    -0.0008337442995980382,
                    0.0010442184284329414,
                    -0.0009920650627464056,
                    0.9999986290931702,
                ],
            ),
    ...
    print(
        "\n".join(
            [
                (
                    "{:<%dd}{:<%ds}{:>%ds} : {:<s}"
                    % (col_0_width, col_1_width, col_2_width)
                ).format(idx, test[0], "OK" if test[1][0] else "FAILED", test[1][1])
...
def get_tf(tf_listener, from_id, to_id):
    ...
    try:
        ...
        tf_listener.waitForTransform(
            from_id, to_id, rospy.Time(), rospy.Duration(tf_timeout)
        )
    ...
    for params_str in params_strs:
            ...
            is_node_up = (
                len(
                    [
                        ...
                        if "realsense2_camera/" in service
                    ]
                )
                > 0
            )
        ...
        if is_node_up:
            ...
            if "static_tf" in [test["type"] for test in rec_tests]:
                ...
                listener_res["static_tf"] = dict(
                    [
                        (xx, get_tf(tf_listener, xx[0], xx[1]))
                        for xx in itertools.combinations(frame_ids, 2)
                    ]
                )
            print("*" * 8 + " Killing ROS " + "*" * 9)
            p_wrapper.terminate()
...
def main():
    ...
    all_tests = [
        {
            ...
            "type": "no_file",
            "params": {"rosbag_filename": "/home/non_existent_file.txt"},
        },
        {
            "name": "vis_avg_2",
            "type": "vis_avg",
            "params": {"rosbag_filename": outdoors_filename},
        },
        {
            "name": "depth_avg_1",
            "type": "depth_avg",
        ...
        {
            "name": "depth_avg_decimation_1",
            "type": "depth_avg_decimation",
            "params": {"rosbag_filename": outdoors_filename, "filters": "decimation"},
        },
    ...
    for test in all_tests:
        test["params"]["rosbag_filename"] = os.path.abspath(
            test["params"]["rosbag_filename"]
        )
        test["params_str"] = " ".join(
            [key + ":=" + test["params"][key] for key in sorted(test["params"].keys())]
        )
