...
class CWaitForMessage:
    def __init__(self, params={}):
        ...
        self.themes = {
            "depthStream": {
                ...
                "callback": self.imageColorCallback,
                "msg_type": msg_Image,
            },
            "colorStream": {
            ...
            "pointscloud": {
                ...
                "callback": self.pointscloudCallback,
                "msg_type": msg_PointCloud2,
            },
            "alignedDepthInfra1": {
                "topic": "/camera/aligned_depth_to_infra1/image_raw",
                "callback": self.imageColorCallback,
                "msg_type": msg_Image,
            },
            "alignedDepthColor": {
    ...
    def imageColorCallback(self, theme_name):
        def _imageColorCallback(data):
            ...
            self.func_data[theme_name]["reported_size"].append(
                (data.width, data.height, data.step)
            )
        ...
        def _pointscloudCallback(data):
            ...
            try:
                points = np.array(
                    [
                        pc2_to_xyzrgb(pp)
                        for pp in pc2.read_points(
                            data, skip_nans=True, field_names=("x", "y", "z", "rgb")
                        )
                        if pp[0] > 0
                    ]
                )
        ...
        if out_filename:
            ...
            if msg_type is msg_Imu:
                ...
                line = (
                    "{:<%d}"
                    * len(columns)
                    % (col_w, col_w, col_w, col_w, col_w, col_w, col_w, col_w)
                ).format(*columns) + "\n"
    ...
    def callback(self, data):
        ...
        if any(
            [
                self.seq < 0 and self.time is None,
                self.seq > 0 and data.header.seq >= self.seq,
                self.time
                and data.header.stamp.secs == self.time["secs"]
                and data.header.stamp.nsecs == self.time["nsecs"],
            ]
        ):
            self.result = data
            self.sub.unregister()
...
def main():
    ...
    if "points" in wanted_topic:
        ...
        ("imu" in wanted_topic) or ("gyro" in wanted_topic) or ("accel" in wanted_topic)
    ):
        msg_type = msg_Imu
    ...
    for idx in range(2, len(sys.argv)):
        ...
        if sys.argv[idx] == "--time":
            msg_params["time"] = dict(
                zip(
                    ["secs", "nsecs"],
                    [int(part) for part in sys.argv[idx + 1].split(".")],
                )
            )
