...
class PointCloudTransformer:
    def __init__(self):
        ...
        self.pub_merged = rospy.Publisher(
            "/merged_point_cloud", PointCloud2, queue_size=10
        )
        self.sub_front = rospy.Subscriber(
            "/laser_scanner_front_cart", PointCloud2, self.transform_callback_front
        )
        self.sub_rear = rospy.Subscriber(
            "/laser_scanner_rear_cart", PointCloud2, self.transform_callback_rear
        )
    ...
    def transform_callback_front(self, cloud_msg):
        try:
            merge_frame = rospy.get_param("~merge_frame", "laserFront")
            transform = self.tf_buffer.lookup_transform(
                merge_frame, cloud_msg.header.frame_id, rospy.Time()
            )
    ...
    def check_and_publish(self):
        if self.front_points is not None and self.rear_points is not None:
            self.merge_points()
            merged_cloud = point_cloud2.create_cloud_xyz32(
                self.current_header, self.merged_points
            )
    ...
    def merge_points(self):
        ...
        for point in point_cloud2.read_points(
            self.front_points, field_names=("x", "y", "z")
        ):
            merged_points.append(point)
        for point in point_cloud2.read_points(
            self.rear_points, field_names=("x", "y", "z")
        ):
            merged_points.append(point)
