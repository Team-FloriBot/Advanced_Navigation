...
class PointCloudTransformer:
    def __init__(self):
        ...
        self.pub_front = rospy.Publisher(
            "/transformed_point_cloud_front", PointCloud2, queue_size=10
        )
        self.pub_rear = rospy.Publisher(
            "/transformed_point_cloud_rear", PointCloud2, queue_size=10
        )
    ...
    def transform_callback_front(self, cloud_msg):
        try:
            transform = self.tf_buffer.lookup_transform(
                "base_link", cloud_msg.header.frame_id, rospy.Time()
            )
