...
class FieldRobotNavigator:
    ...
    def drive_in_row(self):
        ...
        if not len(left_points) >= 10 and not len(right_points) >= 10:
            ...
            left_poly_coeffs = (
                np.polyfit(left_x, left_y, 1) if len(left_points) >= 6 else None
            )
            right_poly_coeffs = (
                np.polyfit(right_x, right_y, 1) if len(right_points) >= 6 else None
            )
            ...
            if left_poly_coeffs is not None:
                ...
                left_dist = (
                    self.row_width - np.polyval(right_poly_coeffs, 0)
            ...
            if right_poly_coeffs is not None:
                ...
                right_dist = (
                    self.row_width - np.polyval(left_poly_coeffs, 0)
                    if left_poly_coeffs is not None
                    else np.inf
                )
    ...
    def turn_to_row(self):
        ...
        y_mean = (
            np.mean((points_y)) if len(points_y) > 0 else np.inf
