...
class MoveRobotPathPattern:
    def __init__(self):
        self.real_sim_parameter = rospy.get_param(
            "~real_sim_parameter"
        )  # Get real_sim_parameter to determine if the script is running on a real robot or in a simulation
        if self.real_sim_parameter == 0:  # Real Robot
            self.sub_laser = rospy.Subscriber(
                "sensors/scanFront", LaserScan, self.laser_callback_front, queue_size=3
            )  # [Laserscan] subscriber on /sensors/scanFront
            self.sub_laser = rospy.Subscriber(
                "sensors/scanRear", LaserScan, self.laser_callback_rear, queue_size=3
            )  # [Laserscan] subscriber on /sensors/scanRear
            self.path_pattern = rospy.get_param(
                "~path_pattern"
            )  # [str] path pattern describing the robots trajectory through the field e.g. S-1L-2L-1L-0R-F
            self.path_pattern = self.path_pattern.replace(
                " ", ""
            )  # remove spaces: S 1L 2L 1L 0R F --> S1L2L1L0RF
            self.path_pattern = self.path_pattern[
                1:-1
            ]  # remove S (Start) and F (Finish): S1L2L1L1RF --> 1L2L1L1R

        elif self.real_sim_parameter == 1:  # Simulation
            self.sub_laser = rospy.Subscriber(
                "laser_scanner_front",
                LaserScan,
                self.laser_callback_front,
                queue_size=3,
            )  # [Laserscan] subscriber on /laser_scanner_front
            self.sub_laser = rospy.Subscriber(
                "laser_scanner_rear", LaserScan, self.laser_callback_rear, queue_size=3
            )  # [Laserscan] subscriber on /laser_scanner_rear
        else:
        ...
        )  # [Twist] publisher on /cmd_vel
        self.p_gain_offset_headland = rospy.get_param(
            "~p_gain_offset_headland"
        )  # [1.0] gain of the p-controller applied to offset from an imaginary line for driving within the headland
        self.p_gain_orient_headland = rospy.get_param(
            "~p_gain_orient_headland"
        )  # [1.0] gain of the p-controller applied to orientation from an imaginary line for driving within the headland
        self.max_lin_vel_in_row = rospy.get_param(
            "~max_lin_vel_in_row"
        )  # [m/s] maximum linear velocity for driving within a row
        self.max_lin_vel_in_headland = rospy.get_param(
            "~max_lin_vel_in_headland"
        )  # [m/s] maximum linear velocity for driving within the headland
        self.max_ang_vel_robot = rospy.get_param(
            "~max_ang_vel_robot"
        )  # [rad/s] maximum angluar velocity of robot
        self.lin_vel_turn = rospy.get_param(
            "~lin_vel_turn"
        )  # [m/s] linear x velocity while turns
        self.laser_scanner_coord_gazebo = rospy.get_param(
            "~laser_scanner_coord_gazebo"
        )  # [] parameter if the laserscanner coord is showing in the other direction of the robot coord systen

        self.scan_front = LaserScan()  # [Laserscan] saving the current laser scan
        self.scan_rear = LaserScan()
        self.scan = LaserScan()
        self.x_front_laser_in_base_link = (
            0.328  # [m] x-coordinate of front_laser frame in base_link frame
        )

        self.x_means = collections.deque(
            maxlen=1
        )  # circular buffer for the means of x-coordinates used in state_turn_exit_row and state_turn_enter_row
        self.y_means = collections.deque(
            maxlen=1
        )  # circular buffer for the means of y-coordinates used in state_turn_exit_row and state_turn_enter_row
        ...
        self.state = (
            "state_wait_at_start"  # [str] state that the state machine starts with
        )
        ...
        self.time_start = (
            rospy.Time.now()
        )  # [rospy.Time] timestamp used in state_headlands
        self.time_exit_row = (
            rospy.Time.now()
        )  # [rospy.Time] timestamp used in state_turn_exit_row
        self.laser_box_drive_headland = np.zeros(
            (10, 2)
        )  # [x, y] Initialising the laserbox
        self.laser_box_detect_row = np.zeros((10, 2))
        self.laser_box_drive_row = np.zeros((10, 2))
        self.xy_scan_raw = np.zeros((10, 2))
        self.there_was_row = (
            False  # [Bool] Variables for determing the transitions between rows
        )
    ...
    def detect_row_end(self):
        ...
        self.laser_box_drive_row = self.laser_box(
            self.scan_front, x_min, x_max, y_min, y_max
        )
        self.x_mean = np.mean(self.laser_box_drive_row[0, :])
        end_of_row = (
            self.x_mean < 0.3
        )  # self.x_front_laser_in_base_link #and ~np.isnan(self.x_mean)
        return end_of_row

    def detect_robot_running_crazy(
        self, collisions_thresh, collision_reset_time, sample_time
    ):
        """
        ...
        if rospy.Time.now() - self.time_collision_detection > rospy.Duration.from_sec(
            sample_time
        ):
        ...
        if collision_rate == 0:
            if (
                rospy.Time.now() - self.time_start_reset_scan_dots
                > rospy.Duration.from_sec(collision_reset_time)
            ):
    ...
    def state_drive_to_row(self, pub_vel):
        ...
        laser_box_drive_to_row = self.laser_box(
            self.scan_front, 0.0, 0.4, -self.row_width, self.row_width
        )
    ...
    def state_in_row(self, pub_vel):
        ...
        self.scan_left_front = self.laser_box(
            self.scan_front, 0.0, 1.0, self.robot_width / 2, self.row_width
        )
        self.scan_right_front = self.laser_box(
            self.scan_front, 0.0, 1.0, -self.row_width, -self.robot_width / 2
        )
        ...
        robot_running_crazy = self.detect_robot_running_crazy(
            collisions_thresh=400, collision_reset_time=2.0, sample_time=0.2
        )
    ...
    def state_turn_exit_row(self, pub_vel):
        ...
        self.laser_box_drive_headland = self.laser_box(
            self.scan_front, x_min, x_max, y_min, y_max
        )
    ...
    def state_headlands(self, pub_vel):
        ...
        self.laser_box_drive_headland = self.laser_box(
            self.scan_front,
            x_min_drive_headland,
            x_max_drive_headland,
            y_min_drive_headland,
            y_max_drive_headland,
        )
        self.x_means.append(np.mean(self.laser_box_drive_headland[0, :]))
        ...
        self.laser_box_detect_row = self.laser_box(
            self.scan_front,
            x_min_detect_row,
            x_max_detect_row,
            y_min_detect_row,
        ...
        if target_row_reached:
            ...
            if turn_sign >= 0:  # left turn
                error_offset = setpoint_offset - np.min(
                    self.laser_box_drive_headland[1, :]
                )
            else:  # right turn
                error_offset = setpoint_offset - np.max(
                    self.laser_box_drive_headland[1, :]
                )
            ...
            print(
                "miny",
                np.min(self.laser_box_drive_headland[1, :]),
                "error_offset",
                error_offset,
                "act_offset",
                act_offset,
