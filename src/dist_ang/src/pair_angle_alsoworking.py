...
class ClosestPairAngleCalculator:
    def __init__(self):
        ...
        self.line_marker_pub = rospy.Publisher("/line_visualization", Marker, queue_size=10)
        self.point_marker_pub = rospy.Publisher("/point_visualization", Marker, queue_size=10)
    ...
    def point_cloud_callback(self, msg):
        ...
        points = [Point32(p[0], p[1], p[2]) for p in point_cloud2.read_points(msg, field_names=("x", "y", "z"), skip_nans=True)]
        
        # Select the closest 10 points to the origin
        closest_points = sorted(points, key=lambda p: p.x**2 + p.y**2 + p.z**2)[:30]
        ...
        for i, point1 in enumerate(closest_points[:-1]):
            for point2 in closest_points[i+1:]:
                ...
                if self.threshold < dist < self.min_distance:
                    angle_rad = math.atan((point2.y - point1.y) / (point2.x - point1.x)) if (point2.x - point1.x) != 0 else (math.pi / 2 if dy > 0 else -math.pi / 2)
                    valid_angles.append(angle_rad)
                    # Update visualization markers
                    self.update_markers(line_list_marker, point_list_marker, point1, point2)
