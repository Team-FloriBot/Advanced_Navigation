...
class ClosestPairAngleCalculator:
    def __init__(self):
        ...
        # Initialize variables for plot update
        self.valid_angles_degrees = []
        self.average_angle_deg = None
    ...
    def process_valid_pairs(self, valid_angles, line_list_marker, point_list_marker):
        if valid_angles:
            ...
            if angles_in_max_bin:
                ...
                self.average_angle_deg = math.degrees(average_angle_rad)
                rospy.loginfo(
                    f"Average angle of max bin (degrees): {self.average_angle_deg}"
                )
                self.closest_pair_angle_pub.publish(self.average_angle_deg)
                self.line_marker_pub.publish(line_list_marker)
                self.point_marker_pub.publish(point_list_marker)

                # Store valid_angles in degrees for plotting
                self.valid_angles_degrees = [
                    math.degrees(angle) for angle in valid_angles
                ]

    def update_plot(self):
        if self.valid_angles_degrees:
            self.ax.clear()
            self.ax.hist(
                self.valid_angles_degrees, bins="auto", color="blue", alpha=0.7
            )
            self.ax.set_title("Histogram of Valid Angles (Degrees)")
            self.ax.set_xlabel("Angle (degrees)")
            self.ax.set_ylabel("Frequency")

            if self.average_angle_deg is not None:
                self.ax.axvline(
                    self.average_angle_deg, color="red", linestyle="dashed", linewidth=1
                )
                self.ax.text(
                    self.average_angle_deg,
                    self.ax.get_ylim()[1] * 0.95,
                    f"Avg: {self.average_angle_deg:.2f}°",
                    rotation=0,
                    color="red",
