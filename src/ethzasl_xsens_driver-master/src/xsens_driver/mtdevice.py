...
class MTDevice(object):
    ...
    def __init__(
        ...
        try:
            self.device = serial.Serial(
                port, baudrate, timeout=timeout, writeTimeout=timeout
            )
        except IOError:
    ...
    def write_msg(self, mid, data=b""):
        ...
        if self.verbose:
            print(
                "MT: Write message id 0x%02X (%s) with %d data bytes: "
                "[%s]"
                % (
                    mid,
                    getMIDName(mid),
    ...
    def read_data_msg(self, buf=bytearray()):
        ...
        while (time.time() - start) < self.timeout:
            ...
            if 0xFF & sum(buf[1:]):
                if self.verbose:
                    sys.stderr.write(
                        "MT: invalid checksum; discarding data "
    ...
    def read_msg(self):
        ...
        while (time.time() - start) < self.timeout:
            ...
            if 0xFF & sum(data, 0xFF + mid + length + checksum):
                if self.verbose:
                    sys.stderr.write(
                        "invalid checksum; discarding data and "
                        "waiting for next message.\n"
                    )
                continue
            if self.verbose:
                print(
                    "MT: Got message id 0x%02X (%s) with %d data bytes: "
                    "[%s]"
        ...
        for _ in range(n_resend):
            ...
            for _ in range(n_read):
                ...
                if mid_ack == (mid + 1):
                    ...
                    print(
                        "ack (0x%02X) expected, got 0x%02X instead" % (mid + 1, mid_ack)
                    )
    ...
    def GetSyncSettings(self):
        ...
        sync_settings = [
            struct.unpack("!BBBBHHHH", data[o : o + 12])
    ...
    def SetSyncSettings(self, sync_settings):
        ...
        self._ensure_config_state()
        data = b"".join(
            struct.pack("!BBBBHHHH", *sync_setting) for sync_setting in sync_settings
        )
    ...
    def GetOutputConfiguration(self):
        ...
        output_configuration = [
            struct.unpack("!HH", data[o : o + 4]) for o in range(0, len(data), 4)
        ]
    ...
    def GetAlignmentRotation(self, parameter):
        ...
        if len(data) == 16:  # fix for older firmwares
            ...
            raise MTException(
                "Could not parse ReqAlignmentRotationAck message:"
    ...
    def ReqDataLength(self):  # deprecated
        ...
        try:
            ...
            if e.code == 0x04:
                sys.stderr.write(
                    "ReqDataLength message is deprecated and not "
    ...
    def GetLatLonAlt(self):
        ...
        if len(data) == 24:
            ...
            raise MTException(
                "Could not parse ReqLatLonAltAck message: wrong" "size of message."
            )
        return (lat, lon, alt)
    ...
    def GetAvailableScenarios(self):
        ...
        try:
            for i in range(len(data) // 22):
                scenario_type, version, label = struct.unpack(
    ...
    def GetUTCTime(self):
        ...
        ns, year, month, day, hour, minute, second, flag = struct.unpack(
            "!IHBBBBBB", data
        )
    ...
    def IccCommand(self, command):
        ...
        if cmd_ack != command:
            raise MTException(
                "expected ack of command 0x%02X; got 0x%02X "
    ...
    def parse_MTData2(self, data):
        ...
        def parse_acceleration(data_id, content, ffmt):
            ...
            if (data_id & 0x00F0) == 0x10:  # Delta V
                o["Delta v.x"], o["Delta v.y"], o["Delta v.z"] = struct.unpack(
                    "!" + 3 * ffmt, content
                )
                ...
                o["freeAccX"], o["freeAccY"], o["freeAccZ"] = struct.unpack(
                    "!" + 3 * ffmt, content
                )
        ...
        def parse_position(data_id, content, ffmt):
            ...
            if (data_id & 0x00F0) == 0x10:  # Altitude MSL  # deprecated
                ...
                o["ecefX"], o["ecefY"], o["ecefZ"] = struct.unpack(
                    "!" + 3 * ffmt, content
                )
        ...
        def parse_SCR(data_id, content, ffmt):
            ...
            if (data_id & 0x00F0) == 0x10:  # ACC+GYR+MAG+Temperature
                ...
                o["tempGyrX"], o["tempGyrY"], o["tempGyrZ"] = struct.unpack(
                    "!hhh", content
                )
            else:
                raise MTException("unknown packet: 0x%04X." % data_id)
        ...
        while len(data) > 0:
            try:
                ...
                raise MTException(
                    "couldn't parse MTData2 message (data_id: "
...
def find_baudrate(port, timeout=0.002, verbose=False, initial_wait=0.1):
    ...
    for br in baudrates:
        ...
        try:
            mt = MTDevice(
                port, br, timeout=timeout, verbose=verbose, initial_wait=initial_wait
            )
