...
def get_param(name, default):
    try:
        ...
        v = default
        rospy.logwarn(
            "Cannot find value for parameter: %s, assigning "
...
class XSensDriver(object):
    def __init__(self):
        ...
        if device == "auto":
            ...
            if devs:
                ...
                return
        if not baudrate:
            baudrate = mtdevice.find_baudrate(
                device, timeout=timeout, initial_wait=initial_wait
            )
        if not baudrate:
        ...
        self.orientation_covariance = matrix_from_diagonal(
            get_param_list(
                ...
                [radians(1.0), radians(1.0), radians(9.0)],
            )
        )

        self.diag_msg = DiagnosticArray()
        self.stest_stat = DiagnosticStatus(
            name="mtnode: Self Test", level=1, message="No status information"
        )
        self.xkf_stat = DiagnosticStatus(
            name="mtnode: XKF Valid", level=1, message="No status information"
        )
        self.gps_stat = DiagnosticStatus(
            name="mtnode: GPS Fix", level=1, message="No status information"
        )
    ...
    def reset_vars(self):
        self.imu_msg = Imu()
        self.imu_msg.orientation_covariance = (-1.0,) * 9
        self.imu_msg.angular_velocity_covariance = (-1.0,) * 9
        self.imu_msg.linear_acceleration_covariance = (-1.0,) * 9
        self.pub_imu = False
    ...
    def spin_once(self):
        ...
        def convert_quat(q, source, dest=self.frame_local):
            ...
            q_enu_ned = (0, 1.0 / sqrt(2), 1.0 / sqrt(2), 0)
            q_enu_nwu = (1.0 / sqrt(2), 0, 0, -1.0 / sqrt(2))
            q_ned_nwu = (0, -1, 0, 0)
            q_ned_enu = (0, -1.0 / sqrt(2), -1.0 / sqrt(2), 0)
            q_nwu_enu = (1.0 / sqrt(2), 0, 0, 1.0 / sqrt(2))
            q_nwu_ned = (0, 1, 0, 0)
            if source == "ENU":
                if dest == "ENU":
                    return q
            ...
            try:
                ...
                if self.last_delta_q_time is None:
                    ...
                    if self.delta_q_rate is None:
                        self.delta_q_rate = 1.0 / delta_t
                    delta_t_filtered = 0.95 / self.delta_q_rate + 0.05 * delta_t
                    # rate in necessarily integer
                    self.delta_q_rate = round(1.0 / delta_t_filtered)
                    self.last_delta_q_time = now
        ...
        def fill_from_Velocity(o):
            ...
            x, y, z = convert_coords(o["velX"], o["velY"], o["velZ"], o["frame"])
            self.vel_msg.twist.linear.x = x
            self.vel_msg.twist.linear.y = y
            self.vel_msg.twist.linear.z = z
            self.pub_vel = True
