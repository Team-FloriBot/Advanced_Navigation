...
model_haag = tf.keras.models.load_model(
    (haag_path), custom_objects={"KerasLayer": hub.KerasLayer}
)
model_haag.summary()
...
def detection():
    ...
    while True:
        ...
        color_image_rgb_crop = color_image_rgb
        ...
        if len(faces_vgg) > 0 or len(faces_cascade) > 0 or haag_pred == 1:
            predict_class2 += 1
            # print("Human")

        elif (
            class_name
            in ["impala", "deer", "gazelle", "wallaby", "Italian_greyhound", "ibex"]
            or haag_pred == 0
        ):
            predict_class1 += 1
            # print("Deer")

        if predict_class2 >= 2:  # class_threshold:
            predict_class1 = 0
            predict_class2 = 0
            predict_false = 0
        ...
        elif predict_class1 >= class_threshold:
            predict_class1 = 0
            predict_class2 = 0
            predict_false = 0
