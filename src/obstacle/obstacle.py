...
class FieldRobotNavigator:
    def __init__(self):
        ...
        self.automatic_mode_pub = rospy.Publisher(
            "/obstacle/obstacle_automatic_mode", Bool, queue_size=1
        )
    ...
    def point_cloud_callback(self, msg):
        ...
        for p in point_cloud2.read_points(
            ...
            point = Point32(*p)
            if (
                self.y_min_front < np.abs(point.y) < self.y_max_front
                and self.x_min_front < point.x < self.x_max_front
            ):
